...
use App\Exports\AdjustmentBranchExport;
use App\Exports\AdjustmentGlobalExport;
...
class DownloadAdjustmentController extends Controller
{
    //
    public function index(Request $request){
        ...
        if ($from_date != '' && $to_date != '') {
            if ($to_date < $from_date) {
                ...
                if ($firstchar != 0) {
                    if ($expexcel) {
                        return Excel::download(new AdjustmentBranchExport($from_date, $to_date, $altbranchcity), 'AdjustmentKTA-' . $yeuh . '-' . $altbranch . '.xlsx');
                    } else {
                        return Excel::download(new AdjustmentBranchExport($from_date, $to_date, $altbranchcity), 'AdjustmentKTA-' . $yeuh . '-' . $altbranch . '.csv');
                    }
                } else {
                    if ($expexcel) {
                        // dd($from_date);
                        return Excel::download(new AdjustmentGlobalExport($from_date, $to_date), 'AdjustmentKTA-' . $yeuh . '-' . 'HO' . '.xlsx');
                    } else {
                        return Excel::download(new AdjustmentGlobalExport($from_date, $to_date), 'AdjustmentKTA-' . $yeuh . '-' . 'HO' . '.csv');
                    }
                }
            }
        }
        return view('downloadadjustment.index');
    }
}
