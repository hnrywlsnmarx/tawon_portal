...
class ScoringController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($firstchar == 0) {
            $scoring = SubmittedData::where('tbl_log_response.request_code', 'like', "%{$keyword}%")
                ->join('tbl_log_request', 'tbl_log_response.request_code', '=', 'tbl_log_request.request_code')
                ->orderBy('tbl_log_response.insert_date', 'desc')
                ->paginate($pagination);
        }
        ...
        return view('globalscoring.index', [
            'ref_no'    => 'Ref No',
            'scoring' => $scoring,
    ...
    public function store(Request $request)
    {
        ...
        // dd($scoring_result);
        KTAScoringModel::create($request->all());
        AktifitasModel::insert([
            ...
            'nama' => session('nama'),
            'url' => $url,
            'action' => 'Create Scoring KTA for Ref No ' . $ref_no,
            'status' => 'Success',
            'caused_by' => '',
            'created_at' => now(),
            'updated_at' => now(),
        ...
        DetailKTAScoringModel::create($request->all());
        AktifitasModel::insert([
            'nik' => $created_by,
            'email' => session('email'),
            'ip_address' => session('ip_address'),
            'nama' => session('nama'),
            'url' => $url,
            'action' => 'Create Detail Scoring KTA for Ref No ' . $ref_no,
            'status' => 'Success',
            'caused_by' => '',
            'created_at' => now(),
            'updated_at' => now(),
        ...
        return redirect()->back()->with('success', 'Data Scoring tersimpan');
    }
    ...
    public function show($id = 0)
    {
        ...
        $scoring = DB::table('tbl_log_response')
            ->join('tbl_log_request', 'tbl_log_response.request_code', '=', 'tbl_log_request.request_code')
            ->where('tbl_log_response.request_code', $id)
            ->get();
