    ...
    public function index(Request $request){
        ...
        if ($from_date != '' && $to_date != '') {
            if ($to_date < $from_date) {
                ...
                if ($firstchar != 0) {
                    if ($expexcel) {
                        return Excel::download(new ApprovalBranchExport($from_date, $to_date, $altbranchcity), 'ApprovalKTA-' . $yeuh . '-' . $altbranch . '.xlsx');
                    } else {
                        return Excel::download(new ApprovalBranchExport($from_date, $to_date, $altbranchcity), 'ApprovalKTA-' . $yeuh . '-' . $altbranch . '.csv');
                    }
                } else {
                    if ($expexcel) {
                        // dd($from_date);
                        return Excel::download(new ApprovalGlobalExport($from_date, $to_date), 'ApprovalKTA-' . $yeuh . '-' . 'HO' . '.xlsx');
                    } else {
                        return Excel::download(new ApprovalGlobalExport($from_date, $to_date), 'ApprovalKTA-' . $yeuh . '-' . 'HO' . '.csv');
                    }
                }
            }
        }
