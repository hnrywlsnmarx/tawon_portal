...
class DownloadPermohonanController extends Controller
{
    //
    public function index(Request $request){
        ...
        if ($from_date != '' && $to_date != '') {
            if ($to_date < $from_date) {
                ...
                if ($firstchar != 0) {
                    if ($expexcel) {
                        return Excel::download(new PermohonanBranchExport($from_date, $to_date, $altbranchcity), 'PengajuanKTA-' . $yeuh . '-' . $altbranch . '.xlsx');
                    } else {
                        return Excel::download(new PermohonanBranchExport($from_date, $to_date, $altbranchcity), 'PengajuanKTA-' . $yeuh . '-' . $altbranch . '.csv');
                    }
                } else {
                    if ($expexcel) {
                        // dd($from_date);
                        return Excel::download(new PermohonanGlobalExport($from_date, $to_date), 'PengajuanKTA-' . $yeuh . '-' . 'HO' . '.xlsx');
                    } else {
                        return Excel::download(new PermohonanGlobalExport($from_date, $to_date), 'PengajuanKTA-' . $yeuh . '-' . 'HO' . '.csv');
                    }
                }
            }
        }
        return view('downloadpermohonan.index');
    }
}
