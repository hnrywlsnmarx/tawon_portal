...
<div class="left-content">
    <h4 class="content-title mb-1">Today Approval & Rejection KTA</h4>				
    <nav aria-label="breadcrumb">					
        <ol class="breadcrumb">				
            <li class="breadcrumb-item"><a href="#">Today Approval & Rejection KTA</a></li>					
...
@section('content')
    ...
    <table class="table table-bordered table-hover mb-0 text-md-nowrap border" style="font-size: smaller;">
        <tr style="background-color: rgb(204, 227, 253)">
            ...
            <th>Scored By HO</th>
            <th>Status by Cabang</th>
            <th>Comment by Cabang</th>
            <th>Created by Cabang</th>
        ...
        <tr>
            ...
            @else
                <td>Reject</td>
            @endif
            <td>{{ $score->created_by_ho }}</td>
                @if($score->status_by_cabang == 'Reject')
                    <td style="background-color: red; font-weight: bolder; color: white;">{{ $score->status_by_cabang }}</td>
                @else
                    <td>{{ $score->status_by_cabang }}</td>
                @endif
            <td>{{ $score->comment_by_cabang }}</td>
            <td>{{ $score->created_by_cabang }}</td>
            <td>
                <a class="btn btn-sm btn-success" href="{{ route('dataapproval.show',$score->ref_no) }}"style="margin-right: 30px">Show</a>
            </td>
        </tr>
        @endforeach
    </table>
