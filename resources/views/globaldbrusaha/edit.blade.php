...
<div class="left-content">				
    <h5 class="content-title mb-1">Edit DBR Usaha</h5>					
    <nav aria-label="breadcrumb">					
        <ol class="breadcrumb">					
            <li class="breadcrumb-item"><a href="#">Edit DBR Usaha</a></li>					
            <li class="breadcrumb-item active" aria-current="page">Show</li>					
        </ol>					
    </nav>			
    <div class="pull-right mt-1">
        <a class="btn btn-sm btn-primary" href="{{ route('dbrusaha.show', $ref_no) }}"> Back</a>
    </div>
</div>
...
@section('content')
    ...
    @endforeach
<form action="{{ route('dbrusaha.update', $id) }}" method="POST">
    @method('PUT')
    @csrf
    <div class="row">
        ...
        if(valSlik == ''){
            ...
            alert('Isi Maturity');
                ...
                if(tglSlik < 15){
                    ...
                    const pmt = ((valBalance - futureValue) * realRate / (1 - Math.pow((1 + realRate), -datediff)));
                    const toLocalePmt = pmt.toLocaleString("id-ID", {style:"decinal"});
                    $('[name="installment"]').val(toLocalePmt);
                    $('[name="installment_number"]').val(pmt);
                }
            }  
