...
<div class="left-content">
    <h4 class="content-title mb-1">Data KTA Expired (Proses terhenti di Cabang)</h4>				
    <nav aria-label="breadcrumb">					
        <ol class="breadcrumb">				
            <li class="breadcrumb-item"><a href="#">Data KTA Expired (Proses terhenti di Cabang)</a></li>					
...
@section('content')
    ...
    <table class="table table-bordered table-hover mb-0 text-md-nowrap border" style="font-size: smaller;">
        ...
        <tr>
            ...
            {{-- <td>{{ $score->jumlah_pinjaman }}</td> --}}
            <td>{{ $score->adjustment_pinjaman }}</td>
            <td>{{ $score->desired_branch }}</td>
            <td>{{ $score->created_by }}</td>
            
            <td>
                <a class="btn btn-sm btn-primary" href="{{ route('dataexpired.show',$score->ref_no) }}" style="margin-right: 30px">Show</a>
            </td>
        </tr>
        @endforeach
    </table>
